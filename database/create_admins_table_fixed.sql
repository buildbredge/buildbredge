-- 创建管理员表 - BuildBridge Admin Table (Fixed Version)
-- 在Supabase控制台的SQL编辑器中执行此脚本

-- 首先删除现有的管理员表和相关对象（如果存在）
...
-- 暂时禁用RLS以避免循环引用问题
-- 在生产环境中，你可能需要更复杂的RLS策略或使用服务角色
ALTER TABLE admins DISABLE ROW LEVEL SECURITY;
ALTER TABLE admin_activity_logs DISABLE ROW LEVEL SECURITY;

-- 如果你需要启用RLS，可以使用以下简化的策略：
-- ALTER TABLE admins ENABLE ROW LEVEL SECURITY;
-- ALTER TABLE admin_activity_logs ENABLE ROW LEVEL SECURITY;

-- 简单的RLS策略（可选 - 仅在需要时启用）
-- CREATE POLICY "Allow service role access" ON admins FOR ALL USING (true);
-- CREATE POLICY "Allow service role access" ON admin_activity_logs FOR ALL USING (true);
...
SELECT email, name, role, department FROM admins;