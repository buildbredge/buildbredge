...
BuildBridge - 连接业主与专业技师的平台`,
  ...
  // 新增：业主资金释放通知邮件
  ownerFundsReleasedNotification: (
    data: OwnerFundsReleasedNotificationEmail,
  ) => ({
...
class SMTPEmailService {
  ...
  private async getTransporter() {
    ...
    if (!smtpConfig.host || !smtpConfig.auth.user || !smtpConfig.auth.pass) {
      throw new Error(
        "SMTP 配置不完整。请检查环境变量：SMTP_HOST, SMTP_USER, SMTP_PASS",
      );
    }
  ...
  // 新增：发送业主资金释放通知邮件
  async sendOwnerFundsReleasedNotification(
    data: OwnerFundsReleasedNotificationEmail,
  ) {
